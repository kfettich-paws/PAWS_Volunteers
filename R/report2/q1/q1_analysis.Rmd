...
p <- ggplot(data = v, aes(x=factor(v$row), y=Median)) +
  ggtitle("Days between orientation and date entered into Volgistics 2018") + 
...
p <- ggplot(data = v, aes(x=factor(v$row), y=Median)) +
  ggtitle("Days between orientation and date of first shift 2018") + 
  geom_bar(stat="identity", fill="steelblue") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size=12)) +
  scale_x_discrete(name = "Orientation Location and Type", breaks = v$row, labels = as.character(v$orient_type.Primary)) + 
...
new.vols.by.month <- with(v.activity[v.activity$orientation.year=="18",], table(orient_loc.Primary, orientation.month, exclude = NULL) )
...
# add months 7-12 wuth NA

new.vols.by.month$Freq[new.vols.by.month$orientation.month==7] <- NA
new.vols.by.month$orientation.month <- as.numeric(as.character(new.vols.by.month$orientation.month))

empty.rows.GF <- data.frame(orient_loc.Primary = c("GF"),
                         orientation.month = c(7,8,9,10,11,12),
                         Freq = NA)
empty.rows.NE <- data.frame(orient_loc.Primary = c("NE"),
                         orientation.month = c(7,8,9,10,11,12),
                         Freq = NA)
empty.rows.PAC <- data.frame(orient_loc.Primary = c("PAC"),
                         orientation.month = c(7,8,9,10,11,12),
                         Freq = NA)
new.vols.by.month <- rbind(new.vols.by.month,empty.rows.GF,empty.rows.NE,empty.rows.PAC)
new.vols.by.month <- new.vols.by.month[!is.na(new.vols.by.month$orient_loc.Primary) & 
                                         !is.na(new.vols.by.month$orientation.month),]
...
p <- ggplot(data=new.vols.by.month[new.vols.by.month$orient_loc.Primary=="GF",],
  ...
  ggtitle("Percent new volunteers attending first orientation at Grays Ferry in 2018") + 
...
p <- ggplot(data=new.vols.by.month[new.vols.by.month$orient_loc.Primary=="PAC",],
       ...
       geom_line() +
  scale_y_continuous(name="Percent", limits=c(0, 30), breaks = seq(0, 20,2)) +
  ggtitle("Percent new volunteers attending first orientation at PAC in 2018") + 
  scale_x_discrete(name = "Month")
...
p <- ggplot(data=new.vols.by.month[new.vols.by.month$orient_loc.Primary=="NE",],
       ...
       geom_line() +
  scale_y_continuous(name="Percent", limits=c(0, 30), breaks = seq(0, 20,2)) +
  ggtitle("Percent new volunteers attending first orientation at Grant Ave in 2018") + 
  scale_x_discrete(name = "Month")
