...
```{r}
#2017 data
(lm.input <- as.formula(paste("nTimesVolunteer", "~", paste(var.min, collapse = "+")))) # prepare for lm fomula
...
#2018 data
(lm.input2 <- as.formula(paste("nTimesVolunteer", "~", paste(var.min2, collapse = "+")))) # prepare for lm fomula


fit.min.lm2 <- lm(lm.input2, data=v18)
...
Furthermore, volunteers who completed shifts at the PAWS South Philly PetSmart, PAC, and at 'other' locations had greater number of times Volunteering. Also, as we might expected, YTD.hours is highly positively associated with number of times volunteering.
