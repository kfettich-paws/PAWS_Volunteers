...
This Rmd contains data cleaning code and some initial plots. The final_report_q3.Rmd has the analysis that uses the output from this file.
...
```{r basic_signup}
knitr::kable(table(signup$Type, exclude=NULL), caption = "Type")
knitr::kable(table(signup$NOT.Attending, exclude=NULL), caption = "Not Attending")
knitr::kable(table(signup$Present, exclude=NULL), caption = "Present")
knitr::kable(table(signup$orientation, exclude=NULL), caption = "Primary or Secondary Orientation")
```
...
The earliest orientation date was `r min(signup_clean$Orientation.Date, na.rm = T)`

Decided to make separate variables for Primary and Secondary orientation (convert to wide). 
There were some cases where there were duplicate Secondary orientations for a single ID. Kept the earliest.
...
service_clean <- service %>%
  mutate(assignment_group = case_when(
    ...
    Assignment == "PAWS South Philly PetSmart" ~ "PetSmart - PAWS South Philly PetSmart",
...
paste("IDs in master table:",length(unique(master_clean$ID)))
paste("IDs in service table:",length(unique(service_clean$ID)))
paste("IDs in signup table:",length(unique(signup_clean$ID)))
# In all three
paste("IDs in both service and signup:",
      length(intersect(service_clean$ID, signup_clean$ID)))
paste("IDs common to all three:",
...
signup_service <- merge(signup_clean, service_summary, by = "ID", all.x = TRUE) %>%
  ...
  mutate(any_shifts = total_shifts != 0,
         ...
         twoplus_shifts = total_shifts > 1 & total_shifts<10, 
...
signup_service_plot <- final_data %>%
  filter(Orientation.Date.Primary < "2018-05-01") %>%
...
final_data$days_since_last <- as.numeric(as.Date(todays_date) - final_data$last_recorded_date)

final_data$test_period <- case_when(
  final_data$Orientation.Date.Primary >= "2017-01-01" & final_data$Orientation.Date.Primary < "2017-05-01" ~ "2017",
  final_data$Orientation.Date.Primary >= "2018-01-01" & final_data$Orientation.Date.Primary < "2018-05-01" ~ "2018",
  TRUE ~ "other"
)
```
...
hist(as.numeric(model_data$days_since_last), 100,
     main = "Days since last shift for volunteers with 1+ shifts",
     xlab = "Days")
...
```{r}
median_days_since <- median(filter(final_data, total_shifts > 0)$days_since_last)

median_days_between <- median(filter(final_data, total_shifts > 0)$time_bw_shifts, na.rm = T)

```

So looking at all the data including recent orientations (where data is censored), the median days since last shift is around a month (`r median_days_since`), and the median time between shifts is about a week (`r median_days_between`).
...
# ggplot(filter(final_data, total_shifts > 0), aes(x = as.Date(todays_date) -Orientation.Date.Primary, y = days_since_last)) +
#   geom_point() +
#   geom_hline(yintercept = 90, color = "red") + 
...
I will restrict to looking at data from volunteers who did orientation before a certain date (May 01 2018) because we don't know if these volunteers will become disengaged.
...
We will define two test periods to compare:

* 2017: Orientation (primary) between Jan 01 2017 - May 01 2017
* 2018: Orientation (primary) between Jan 01 2018 - May 01 2018

To be a real comparison, we would need to only look at service data between Jan 01 and July 26 from both years as well.

Overall, it looks like more volunteers did orientation in 2018! 
```{r summarise}
end_summary <- final_data  %>%
  group_by(test_period) %>% 
  summarise(total_volunteers = n(),
            volunteered_never = paste( round(100* sum(total_shifts==0)/n(),1) ,"%"),
            volunteered_onlyonce = paste( round(100* sum(total_shifts==1)/n(),1),"%"),
            twototen = paste( round(100* sum(total_shifts>=2 & total_shifts<10)/n(), 1), "%"),
            tenplus = paste( round(100* sum(total_shifts>=10)/n(), 1), "%"),
