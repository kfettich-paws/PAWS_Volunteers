...
service_summary <- service_clean %>%
  ...
  summarise(total_shifts = sum(no_show==FALSE),
            ...
            first_shift_date = min(From.date),
            last_recorded_date = max(From.date),
...
final_data <- merge(signup_service, select(master_clean, ID, Status, city_clean, Zip), by = "ID")
# have some IDs not in master_clean that are in signup_service - these have early date entered

# did a manual clean here that might be controversial
# changed dates for primary orientation for two volunteers that looked like incorrect years
final_data[final_data$ID == "1384", "Orientation.Date.Primary"] <- as.Date("2017-01-21")
final_data[final_data$ID == "1691", "Orientation.Date.Primary"] <- as.Date("2017-01-28")
...
final_data$time_to_first <- final_data$first_shift_date - final_data$Orientation.Date.Primary
