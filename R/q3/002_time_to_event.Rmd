...
final_data$timebw_shift_group <- case_when(
  as.numeric(final_data$time_bw_shifts) <= 7 ~ "<= 7 days",
  as.numeric(final_data$time_bw_shifts) <= 14 ~ "7 to 14 days",
  as.numeric(final_data$time_bw_shifts) > 14 ~ "> 14 days"
)
...
ggsurvplot(surv.fit, data = model_data,
           ...
           palette = brewer.pal(n=9, "Set1"),
           xlab = "Time From First to Last Shift (days)",
           title = "Compare ave time between shifts",
           legend.title = "", legend.labs = levels(factor(model_data$timebw_shift_group)))  

```

I don't understand this, but it looks like volunteers that come frequently also drop out faster than those that come on average less frequently.
